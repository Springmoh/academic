...
/*
 * Argument:ts		Sampling time
			sat		Saturation value, normalized to 1
			ke		Error gain
			ku		Output gain
...
{
	return !pid->flag.s_flag;	/* 1 is not saturated, 0 is saturated */
}
...
void PID (PID_t *pid)
{
	...
	if (pid->K[KI] != 0){
		if (pid->flag.s_flag){     //flag only, so 0 or 1...
...
/*Remarks :
-- pid->s_delay is the output coefficient, i think will be decimal that means smaller than modulus of 1
-- positive value maybe move one direction while negative value move another direction
-- pid->K[SAT] always set as 1, so if more than one no matter which direction, 1 will be coefficient
...
-- if it is not saturated, output coefficient will multiply with max limit that means will get a value that is smaller value than max limit..
*/
